...
public class UserOrchestrationService {
    ...
    private Optional<LdapPerson> ldapFindPerson(LdapConnection ldapConnection, final String nickname) throws org.haiku.haikudepotserver.user.LdapException {
        ...
        try {
            ...
            throw new org.haiku.haikudepotserver.user.LdapException("an error arose finding the user in the ldap system; " + nickname, ce);
        }
    ...
    public void ldapUpdateUser(ObjectContext context, User user) throws org.haiku.haikudepotserver.user.LdapException {
        ...
        if(user.getNickname().equals(User.NICKNAME_ROOT)) {
            ...
            if (isLdapConfigured()) {
                ...
                try {
                    ...
                    throw new org.haiku.haikudepotserver.user.LdapException("unable to update user to ldap directory; " + user.getNickname(), le); // TODO; better exception handling
                } finally {
    ...
    public void ldapSynchronizeAllUsers(ObjectContext context) throws org.haiku.haikudepotserver.user.LdapException {
        Preconditions.checkNotNull(context);
    ...
    public void ldapRemoveNonExistentUsers(ObjectContext context) throws org.haiku.haikudepotserver.user.LdapException {
        ...
        if(isLdapConfigured()) {
            ...
            try {
                ...
                throw new org.haiku.haikudepotserver.user.LdapException("unable to remove non-existent users from the ldap directory", le); // TODO; better exception handling
            } finally {
                if (null != ldapConnection) {
                    try {
                        ldapConnectionPoolHolder.get().releaseConnection(ldapConnection);
