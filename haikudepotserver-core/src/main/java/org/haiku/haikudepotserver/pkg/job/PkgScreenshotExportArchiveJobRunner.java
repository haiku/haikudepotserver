...
public class PkgScreenshotExportArchiveJobRunner
    ...
    private static int BATCH_SIZE = 4; // small because don't want to use too much memory!

    private static int ROW_PKG_NAME = 0;
    private static int ROW_PAYLOAD = 1;
    private static int ROW_PKG_MODIFY_TIMESTAMP = 2;
    ...
    private String createEjbqlRawRowsExpression(PkgScreenshotExportArchiveJobSpecification specification) {
        ...
        builder.append("SELECT psi.pkgScreenshot.pkg.name,\n");
        builder.append("psi.data, psi.pkgScreenshot.pkg.modifyTimestamp,\n");
        builder.append("psi.pkgScreenshot.ordering\n");
        builder.append("FROM " + PkgScreenshotImage.class.getSimpleName() + " psi\n");
        builder.append("WHERE psi.pkgScreenshot.pkg.active = true\n");

        if(!Strings.isNullOrEmpty(specification.getPkgName())) {
            builder.append("AND psi.pkgScreenshot.pkg.name = :pkgName\n");
        }

        builder.append("ORDER BY psi.pkgScreenshot.pkg.name ASC,\n");
