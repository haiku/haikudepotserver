...
A user is required to store an email address with their account. The email address is anticipated to be used for the following purposes;

* As an identifier for integration with the Haiku [single-signon](https://sso.haiku-os.org/) system.
* For administrators or the HDS system to communicate with users using public internet email services.
* For facilitating a user being able to reset their password.

There is no facility in the HDS system intended for publicly publishing users' email addresses.
