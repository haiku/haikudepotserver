...
Considered in this document are both the open-source software of the HDS system and the primary [production deployment](https://depot.haiku-os.org) of that software.
...
- The HDS project is closely coupled to the [Haiku operating system project](https://www.haiku-os.org), but user data relating to any other online or offline services of the larger Haiku project are not considered here; _See later section related to "Future SSO Integration" regarding the Haiku SSO system._

## Application Logs

In order to support the operation of the HDS system and also for diagnosis of issues, HDS will write out logs. The logs may contain
IP-addresses and other personal identifiable information. These logs are only accessed by the administrators to review and diagnose system
...
A Browser is a piece of software that can be run on a computing device in order to view and navigate web-sites and web applications. HDS provides a web application that can be accessed using a browser.

#### Tracking

HDS does not track browsing and does not syndicate browsing behavior with any third parties.

#### Cookies

Browser Cookies are used by HDS to maintain a reference to an HDS server-side user authentication. This is required for the HDS system to function. This type of Browser Cookie is often called a Session Cookie.
...
- Authorisation configuration may be stored for some users authorizing them to perform certain functions in the HDS system that typical users are not able to perform.
...
At a future point in time, the HDS system will be integrated with the [Haiku Single Sign-on](https://sso.haiku-os.org/) (SSO) system. At that time, this user usage conditions document will be republished to reflect those changes.

In the interim, as a preparatory step, some user identifiers from the Haiku SSO system will be integrated into the HDS system as part of a preparation for the migration.

It is later anticipated that the majority of users in the HDS system will be deleted and users will be asked to use their Haiku SSO users to access HDS instead.
